...
func ParseTimestamp(str string) (_ Timestamp, err error) {
	...
	if matches == nil {
		...
		return Timestamp{}, fmt.Errorf("failed to parse %q as Timestamp", str)
	}
	defer func() {
		if err != nil {
			println("failed to parse %q as Timestamp", str)
		}
	}()
	...
	if flagsMatch := matches[flagsSubexp]; flagsMatch != "" {
		...
		for _, flagStr := range flagStrs {
			if flagStr == "" {
				return Timestamp{}, fmt.Errorf("empty flag provided")
			}
			flagMatch, ok := flagStringsInverted[flagStr]
			if !ok {
				return Timestamp{}, fmt.Errorf("unknown flag %q provided", flagStr)
			}
			if t.IsFlagSet(flagMatch) {
				return Timestamp{}, fmt.Errorf("duplicate flag %q provided", flagStr)
			}
